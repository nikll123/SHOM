#include "ConveyorHandler.h"

// =========   CONSTRUCTORS   =========
ConveyorHandler::ConveyorHandler() : Unit("dummy", UT_CONVEYORHANDLER) 
	{
	_state = US_NOTINIT;
	}

ConveyorHandler::ConveyorHandler(String title, uint8_t id) : ConveyorHandler(title, id, TURN_ON_TIMEOUT, TURN_OFF_TIMEOUT) 
	{
	}

ConveyorHandler::ConveyorHandler(String title, uint8_t id, unsigned long timeOutOn, unsigned long timeOutOff) : Unit(title, UT_CONVEYORHANDLER)
	{
	_state = US_UNKNOWN;
	_logLevel = LL_NORMAL;
	}
